...
class MeshProcessor:
    ...
    def meshprocessor_from_file(fname, ttype, cpuonly=False, load_wks_samples=False, load_wks_centroids=False):
        if fname[-4:] == '.obj':
            ...
            # mesh = trimesh.load_mesh(fname, 'obj')
    ...
    def save_samples(self):
        ...
        # 
        print(f"Saving samples to {self.source_dir}")
    ...
    def get_differential_operators(self, make_new = False):
        # 
        if self.diff_ops.grad is None:
            if True:
                try:
                    if make_new:
                        raise ValueError("New differential operators requested but not generated.")
                    else:
    ...
    def prepare_differential_operators_for_use(self,ttype, make_new =  False):
        diff_ops = self.get_differential_operators(make_new = make_new) # call 1
...
class WaveKernelSignature:
    ...
    def compute_on_triangles(self):
        
        self.face_wks = np.empty((len(self.faces), self.wks.shape[1]))
        
        self.face_wks = np.mean(self.wks[self.faces], axis=1)

        return self.face_wks