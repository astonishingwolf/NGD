...
class ClothTexture:
    ...
    def save_weights(self, output_path, epoch = None):
        if epoch is not None:
            torch.save(self.mlp.state_dict(), os.path.join(output_path,'texture_model_weights',f'model_epoch_{epoch}.pth'))
        else:
            torch.save(self.mlp.state_dict(), os.path.join(output_path,'texture_model_weights.pth'))
