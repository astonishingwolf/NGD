...
class SMPL(nn.Module):
    ...
    def __init__(self, model_path, name="smpl"):
        ...
        shape = np.array(dd["shapedirs"][...,:10]).reshape([-1, self.num_shapes]).T
    ...
    def forward(self, shape=None, pose=None, translation=None):
        ...
        if pose is None:
            return vs, torch.zeros((self.num_joints.to(DEVICE), 4, 4))
        ...
        joint_locations_local = torch.stack(
            [
                torch.matmul(vs[:, :, 0], self.joint_regressor.to(DEVICE)),
                torch.matmul(vs[:, :, 1], self.joint_regressor.to(DEVICE)),
                torch.matmul(vs[:, :, 2], self.joint_regressor.to(DEVICE))
            ],
            dim=2
        )
        ...
        pose_blendshape = torch.matmul(pose_feature, self.posedirs.to(DEVICE)).reshape(-1, self.num_vertices, 3)
        ...
        joint_transforms, joint_locations = PoseSkeleton()(
            ...
            self.kintree_table.to(DEVICE)
        )

        # Apply linear blend skinning
        v = LBS()(vp, joint_transforms, self.skinning_weights.to(DEVICE))
